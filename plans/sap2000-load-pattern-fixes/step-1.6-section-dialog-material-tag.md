...
from gui.core.model_data import Section, SectionType, StructuralModel
...
class SectionDialog(QDialog):
    ...
    def __init__(
        ...
        # --- Material asociado (para densidad / peso propio) ---
        self._material_combo = QComboBox()
        self._material_combo.addItem("(Ninguno — sin peso propio)", None)
        if model:
            for mat_tag in sorted(model.materials.keys()):
                mat = model.materials[mat_tag]
                label = f"{mat_tag}: {mat.name} ({mat.density} kg/m³)"
                self._material_combo.addItem(label, mat_tag)
        if section and section.material_tag is not None:
        ...
        form_info.addRow("Material (densidad):", self._material_combo)
    ...
    def get_section(self) -> Section:
        ...
        material_tag = self._material_combo.currentData()
        return Section(
